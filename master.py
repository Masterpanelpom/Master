...
async def init_master_db():
    async with MASTER_DB_LOCK:
        ...
        try:
            ...
            """)
            # Ensure bot_type column exists for existing databases
    ...
    async with TENANT_DB_LOCKS[slug]:
        ...
        try:
            ...
            default_plans = [
                ...
                ("plan_6", "✂️ ʟᴇsʙɪᴀns ✂️", 79.0, "30 Days", ""),
...
class MultiBotEngine:
    ...
    async def start_tenant_bot(self, slug: str, token: str):
        ...
        # Retrieve bot_type from master database to route to the correct template factory
        db = await get_master_db()
        try:
        ...
        # Modular Factory Routing
        if b_type == "pompom_v1":
            self._attach_pompom_v1_handlers(dp, slug)
        else:
            # Fallback to standard Pom Pom Bot V1 template
            self._attach_pompom_v1_handlers(dp, slug)
                        ...
                        <table class="w-full text-left text-xs font-mono">
                            ...
                            <tbody class="divide-y divide-purple-900/30 text-slate-300">
                                ...
                                <tr class="hover:bg-purple-900/20 transition">
                                    ...
                                    <td class="py-2.5 px-3">
                                        <span class="px-2 py-0.5 rounded text-[10px] uppercase font-tech {{ 'text-emerald-400 bg-emerald-500/10' if c.status == 'ACTIVE' and c.days_left > 0 else 'text-rose-500/20 text-rose-400 border border-rose-500/30' }}">
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <!-- TAB: BROADCAST (EXACTLY AS YOU GAVE WITH VIDEO URL FEATURE) -->
            <div id="tab-broadcast" class="tab-content space-y-6 hidden">
                <form method="POST" action="/admin/broadcast/send" class="glass-card rounded-2xl p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs text-purple-300 mb-2 font-mono uppercase">Video URL (Optional)</label>
                        <input type="url" name="broadcast_video" placeholder="https://...mp4" class="w-full bg-[#070410] border border-purple-900/60 rounded-xl px-4 py-2 text-sm text-white focus:outline-none">
                    </div>
...
# ==========================================
# BROADCAST ROUTE (EXACTLY AS YOU GAVE WITH VIDEO URL FEATURE)
# ==========================================
...
async def tenant_broadcast_send(
    ...
    for (uid,) in users:
        try:
            if cleaned_video:
                await bot.send_video(chat_id=uid, video=cleaned_video, caption=broadcast_message)
            elif cleaned_photo:
