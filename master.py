...
# ==========================================
# MASTER LOGIN PAGE (CYBERPUNK VECTOR DESIGN)
# ==========================================
...
<head>
    ...
    <style>
        body {
            ...
            background-image: radial-gradient(circle at 50% 15%, #180533 0%, #080214 55%, #020008 100%);
            min-height: 100vh;
            margin: 0;
            padding: 0;
        }
        ...
        .console-frame {
            ...
            box-shadow: 0 0 20px rgba(0, 240, 255, 0.4), inset 0 0 20px rgba(168, 85, 247, 0.2);
            border-radius: 26px;
            position: relative;
        }
        ...
        .top-badge-crest {
            ...
            height: 52px;
            border-radius: 50%;
            background: radial-gradient(circle, #38bdf8 0%, #a855f7 65%, #150630 100%);
            border: 2px solid #ffffff;
            box-shadow: 0 0 20px #00f0ff, 0 0 35px rgba(236, 72, 153, 0.8);
            display: flex;
            align-items: center;
        ...
        .input-user-box {
            ...
            box-shadow: 0 0 10px rgba(236, 72, 153, 0.25), inset 0 0 8px rgba(236, 72, 153, 0.1);
        }
        .input-pass-box {
            background-color: #080416;
        ...
        .auth-glow-btn:hover {
            opacity: 0.95;
            box-shadow: 0 0 28px rgba(236, 72, 153, 0.7), 0 0 45px rgba(0, 240, 255, 0.45);
        }
    </style>
</head>
<body class="flex flex-col items-center justify-center p-4 min-h-screen">

    <div class="flex items-center gap-2.5 font-tech text-[9px] tracking-[0.25em] text-cyan-300 font-bold uppercase mb-7">
    ...
    <div class="w-full max-w-[360px] relative">
        <div class="top-badge-crest">
            <svg class="w-6 h-6 text-white filter drop-shadow-[0_0_6px_#ffffff]" viewBox="0 0 24 24" fill="currentColor">
                <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/>
            </svg>
        </div>

        <div class="console-frame pt-9 pb-7 px-6 space-y-5">
            <div class="text-center pt-1 space-y-1">
                <h1 class="font-brush-real text-3xl text-pink-400 filter drop-shadow-[0_0_12px_rgba(244,114,182,0.8)] leading-tight tracking-wide">
                    MASTER<br>CONSOLE
                </h1>
                <p class="font-tech text-[8px] tracking-[0.22em] text-cyan-400 font-bold uppercase pt-1">
                    MULTI-TENANT BOT CONTROL &amp; PROVISIONING
                </p>
            </div>

            {% if error %}
            <div class="p-2.5 rounded-xl bg-rose-500/20 border border-rose-500/50 text-rose-300 text-xs font-mono text-center">
                {{ error }}
            </div>
            {% endif %}

            <form method="POST" action="/master/login" class="space-y-4 pt-1">
                <div class="space-y-1">
                    <label class="flex items-center gap-1.5 text-[10px] font-tech font-bold text-cyan-300 uppercase tracking-wider">
                        <svg class="w-3 h-3 text-cyan-400 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="space-y-1">
                    ...
                    <div class="input-pass-box rounded-xl flex items-center px-3 py-2.5 gap-2.5">
                        ...
                        <input type="password" id="passField" name="password" required placeholder="Enter your password"
                               class="w-full bg-transparent text-xs text-white placeholder-slate-500 focus:outline-none font-mono">
                        <button type="button" onclick="togglePassVisibility()" class="text-slate-400 hover:text-cyan-300 focus:outline-none">
    ...
    <!-- Compact Clean Contact Developer Footer Button -->
    <div class="w-full max-w-[360px] mt-4">
        <a href="https://t.me/NAGATOxOWNER" target="_blank" rel="noopener noreferrer"
           class="group flex items-center justify-between p-3 rounded-2xl bg-[#090518]/90 border border-cyan-500/40 hover:border-cyan-400 shadow-[0_0_15px_rgba(0,240,255,0.15)] transition">
            <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-lg bg-cyan-500/10 border border-cyan-400 flex items-center justify-center text-cyan-300">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.95-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.36.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .38z"/>
                    </svg>
                </div>
                <div>
                    <span class="font-tech text-xs font-bold text-white group-hover:text-cyan-300 transition">Contact Developer</span>
                    <span class="block text-[10px] text-pink-400 font-mono">@NAGATOxOWNER</span>
                </div>
            </div>
            <svg class="w-4 h-4 text-cyan-400 group-hover:translate-x-0.5 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
        </a>

        <div class="flex items-center justify-center gap-3 text-[8px] font-tech text-slate-500 tracking-[0.25em] uppercase mt-4">
