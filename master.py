...
class MultiBotEngine:
    ...
    def _attach_handlers(self, dp: Dispatcher, slug: str):
        ...
        # Unified screenshot receiver supporting compressed photos and image files
        ...
        async def process_payment_proof(message: types.Message, state: FSMContext):
            ...
            admin_ids = await get_tenant_admin_ids(slug)
            if not admin_ids:
                logging.warning(f"[{slug}] No admin_chat_id configured! Notification skipped.")
...
# ==========================================
# MASTER LOGIN PAGE (EXACT CYBERPUNK IMAGE MATCH)
# ==========================================
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@600;700;800;900&family=Permanent+Marker&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
    <style>
        body {
            font-family: 'Plus Jakarta Sans', sans-serif;
            background: radial-gradient(circle at top, #14052b 0%, #060212 50%, #020108 100%);
            min-height: 100vh;
            color: #f1f5f9;
        }
        ...
        .neon-shell {
            background: linear-gradient(180deg, rgba(16, 9, 36, 0.92) 0%, rgba(7, 3, 18, 0.98) 100%);
            border: 2px solid #00f0ff;
            box-shadow: 0 0 25px rgba(0, 240, 255, 0.45), inset 0 0 25px rgba(168, 85, 247, 0.25);
            border-radius: 28px;
            position: relative;
        }

        .crest-circle {
            background: radial-gradient(circle, #00f0ff 0%, #a855f7 70%, #0b0320 100%);
            box-shadow: 0 0 25px #00f0ff, 0 0 45px #ec4899;
        ...
        .neon-input-box:focus-within {
            border-color: #ec4899;
            box-shadow: 0 0 15px rgba(236, 72, 153, 0.5), inset 0 0 10px rgba(236, 72, 153, 0.2);
        }

        .neon-auth-btn {
            background: linear-gradient(90deg, #ec4899 0%, #a855f7 50%, #00f0ff 100%);
            box-shadow: 0 0 25px rgba(236, 72, 153, 0.6), 0 0 40px rgba(0, 240, 255, 0.3);
            border: 1px solid rgba(255, 255, 255, 0.4);
            transition: all 0.3s ease;
        }
        .neon-auth-btn:hover {
            opacity: 0.95;
            box-shadow: 0 0 35px rgba(236, 72, 153, 0.8), 0 0 55px rgba(0, 240, 255, 0.5);
        ...
        .dev-badge {
            background: linear-gradient(90deg, rgba(14, 8, 32, 0.95) 0%, rgba(20, 10, 48, 0.95) 100%);
            border: 2px solid #00f0ff;
...
<body class="flex flex-col items-center justify-center min-h-screen p-3 sm:p-6 select-none overflow-x-hidden relative">

    <div class="flex items-center gap-3 font-tech text-[10px] tracking-widest text-cyan-300 font-bold uppercase mb-4">
        <span>FAST</span> &bull; <span>SECURE</span> &bull; <span>RELIABLE</span>
        <div class="flex gap-1 ml-1 text-pink-500">
    ...
    <div class="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        <!-- Left Column: Key Features Badge -->
        <div class="hidden lg:flex lg:col-span-3 flex-col gap-4">
            <div class="font-brush text-pink-500 text-lg -rotate-6 transform">
                MORE THAN<br>JUST A BOT...
            </div>

            <div class="side-card p-4 space-y-4">
                <div class="font-tech text-xs font-black text-cyan-300 tracking-wider uppercase border-b border-pink-500/30 pb-2">
                    KEY FEATURES
                </div>
                ...
                <div class="flex items-start gap-2.5">
                    ...
                    <div>
                        <h4 class="text-[11px] font-bold text-white uppercase font-tech">MULTI-TENANT</h4>
                        <p class="text-[10px] text-slate-400">Manage multiple users easily</p>
                    </div>
                </div>

                <div class="flex items-start gap-2.5">
                    <span class="text-pink-400 mt-0.5 text-sm">🤖</span>
                    <div>
                        <h4 class="text-[11px] font-bold text-white uppercase font-tech">BOT CONTROL</h4>
                        <p class="text-[10px] text-slate-400">Full control over your bots</p>
                    </div>
                </div>

                <div class="flex items-start gap-2.5">
                    <span class="text-cyan-400 mt-0.5 text-sm">🛡️</span>
                    <div>
                        <h4 class="text-[11px] font-bold text-white uppercase font-tech">PROVISIONING</h4>
                        <p class="text-[10px] text-slate-400">Fast & secure isolated access</p>
                    </div>
                </div>

                <div class="flex items-start gap-2.5">
                    <span class="text-purple-400 mt-0.5 text-sm">⚙️</span>
                    <div>
                        <h4 class="text-[11px] font-bold text-white uppercase font-tech">ADMIN PANEL</h4>
                        <p class="text-[10px] text-slate-400">Independent store instances</p>
                    </div>
                </div>
            </div>

            <div class="font-brush text-cyan-400 text-sm pl-2">
                CONTROL<br>MANAGE<br>GROW
            </div>
        </div>

        <!-- Center Column: Main Console Form Card -->
        <div class="lg:col-span-6 w-full max-w-md mx-auto relative pt-8">

            <div class="absolute -top-3 left-1/2 -translate-x-1/2 w-16 h-16 rounded-full crest-circle flex items-center justify-center z-20">
                <svg class="w-8 h-8 text-white filter drop-shadow-[0_0_8px_#ffffff]" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/>
                </svg>
            </div>

            <div class="neon-shell p-6 sm:p-8 pt-10 space-y-6">

                <div class="text-center space-y-1">
                    <h1 class="font-brush text-3xl sm:text-4xl text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-pink-400 to-fuchsia-500 tracking-wider">
                        MASTER<br>CONSOLE
                    </h1>
                    <p class="font-tech text-[10px] tracking-[0.25em] text-cyan-400 uppercase font-bold pt-1">
                        MULTI-TENANT BOT CONTROL &amp; PROVISIONING
                    </p>
                </div>

                {% if error %}
                <div class="p-3 rounded-xl bg-rose-500/15 border border-rose-500/50 text-rose-300 text-xs font-mono text-center shadow-[0_0_15px_rgba(244,63,94,0.3)]">
                    {{ error }}
                </div>
                {% endif %}
                ...
                <form method="POST" action="/master/login" class="space-y-4">
                    <div class="space-y-1.5">
                        <label class="flex items-center gap-2 text-xs font-tech font-bold text-cyan-300 uppercase tracking-wider">
                            <svg class="w-3.5 h-3.5 text-cyan-400" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"/>
                            </svg>
                            MASTER USER
                        </label>
                        <div class="neon-input-box rounded-xl flex items-center px-3.5 py-2.5 gap-2.5">
                            <span class="text-slate-500 text-sm">👤</span>
                            <input type="text" name="username" required autofocus placeholder="Enter your master username"
                                   class="w-full bg-transparent text-sm text-white focus:outline-none placeholder:text-slate-600 font-mono">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="neon-input-box rounded-xl flex items-center px-3.5 py-2.5 gap-2.5">
                            <span class="text-slate-500 text-sm">🔒</span>
                            <input type="password" id="masterPassInput" name="password" required placeholder="Enter your password"
                                   class="w-full bg-transparent text-sm text-white focus:outline-none placeholder:text-slate-600 font-mono">
                            <button type="button" onclick="togglePassVisibility()" class="text-slate-400 hover:text-cyan-300 focus:outline-none text-xs">
                                👁️
                            </button>
                        </div>
                    </div>

                    <button type="submit" class="w-full neon-auth-btn font-tech font-black text-sm uppercase py-3.5 rounded-xl text-white flex items-center justify-center gap-2 tracking-wider mt-2">
                        <span>🛡️</span> AUTHENTICATE &gt;
                    </button>
                </form>
            </div>
        </div>

        <!-- Right Column: Slogan Badge -->
        <div class="hidden lg:flex lg:col-span-3 flex-col items-center justify-center">
            <div class="side-card p-6 text-center space-y-2 border-cyan-400/40 shadow-[0_0_25px_rgba(0,240,255,0.25)]">
                <div class="font-brush text-2xl text-transparent bg-clip-text bg-gradient-to-br from-pink-400 via-purple-300 to-cyan-300 leading-tight">
                    YOUR<br>BOT<br>YOUR<br>RULES
                </div>
    ...
    <!-- Bottom Contact Developer Bar -->
    <div class="w-full max-w-2xl mt-6">
        <div class="dev-badge p-3 sm:p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-cyan-500/20 border border-cyan-400 flex items-center justify-center text-cyan-300 text-lg shadow-[0_0_12px_rgba(0,240,255,0.5)]">
                    ✈️
                </div>
                <div>
                    <h3 class="font-tech text-xs sm:text-sm font-black text-white">
                        CONTACT <span class="text-pink-400">DEVELOPER</span>
                    </h3>
                    <p class="text-[11px] text-slate-400">Have a question? Need help? Reach out!</p>
                </div>
            </div>

            <a href="https://t.me/NAGATOxOWNER" target="_blank" rel="noopener noreferrer"
               class="w-full sm:w-auto px-4 py-2 rounded-xl bg-cyan-500/10 border border-cyan-400/80 hover:bg-cyan-500/20 text-cyan-300 text-xs font-mono font-bold flex items-center justify-center gap-2 transition shadow-[0_0_15px_rgba(0,240,255,0.25)]">
                <span>✈️</span> @NAGATOxOWNER &gt;
            </a>
        </div>

        <div class="flex items-center justify-center gap-4 text-[10px] font-tech text-slate-500 tracking-widest uppercase mt-4">
            <span>BUILD</span> &bull; <span>AUTOMATE</span> &bull; <span>DOMINATE</span>
        </div>
    </div>
...
# ==========================================
# MASTER DASHBOARD (3-LINE MENU & 4 TABS)
# ==========================================
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <!-- Main Content Container with Top 3-Line Menu & Nagato Panel Title -->
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        <header class="sticky top-0 z-20 bg-[#06040c]/95 backdrop-blur-md border-b border-purple-900/40 px-4 md:px-8 py-3.5 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <button type="button" onclick="toggleMasterMenu()" class="p-2 rounded-lg bg-[#120b22] border border-purple-900/40 text-purple-300 hover:text-white focus:outline-none">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                </button>
                <div>
                    <h2 class="font-tech text-base md:text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-fuchsia-300 to-cyan-300 tracking-wider">
                        Nagato Panel
                    </h2>
                </div>
            </div>
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <div id="m-tab-dashboard" class="m-tab-content space-y-6">
                ...
                <div class="glass-card rounded-2xl p-6 space-y-3">
                    <h3 class="font-tech text-sm font-bold text-white border-b border-purple-900/40 pb-2">Master Cloud Provisioner</h3>
                    <p class="text-xs text-slate-300 leading-relaxed font-sans">
                        From here you can provision isolated instances, generate custom client admin panels, regulate plan expiration dates, and control concurrent background bot pollers.
                    </p>
                </div>
            </div>
                        ...
                        <table class="w-full text-left text-xs font-mono">
                            <thead class="text-purple-400 uppercase text-[10px] border-b border-purple-900/40">
                                <tr>
                                    ...
                                    <th class="py-2.5 px-3">Make Date</th>
                            ...
                            <tbody class="divide-y divide-purple-900/30 text-slate-300">
                                ...
                                <tr class="hover:bg-purple-900/20 transition">
                                    ...
                                    <td class="py-2.5 px-3 text-slate-400">{{ c.make_date }}</td>
                                    ...
                                    <td class="py-2.5 px-3">
                                        <span class="px-2 py-0.5 rounded text-[10px] uppercase font-tech {{ 'text-emerald-400 bg-emerald-500/10' if c.status == 'ACTIVE' and c.days_left > 0 else 'text-rose-400 bg-rose-500/10' }}">
                                ...
                                {% else %}
                                <tr><td colspan="8" class="text-center py-8 text-purple-400">No client panels deployed yet.</td></tr>
                                {% endfor %}
                            </tbody>
                        </table>
                    </div>
            ...
            <div id="m-tab-new" class="m-tab-content space-y-6 hidden">
                <div class="glass-card rounded-2xl p-6 space-y-4">
                    ...
                    <form method="POST" action="/master/client/new" class="space-y-4 text-xs font-mono">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="text-purple-300 block mb-1 uppercase">Sell Price (₹)</label>
                            ...
                            <div>
                                <label class="text-purple-300 block mb-1 uppercase">Telegram Bot Token (Optional)</label>
                                <input type="text" name="bot_token" placeholder="123456:ABC-DEF..." class="w-full bg-[#070410] border border-purple-900/60 rounded-xl p-2.5 text-white">
                            </div>
                        </div>
...
# FULL UNALTERED DASHBOARD PAGE TEMPLATE
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <!-- Sidebar with original Nagato Panel branding -->
                ...
                <div class="glass-card rounded-2xl p-5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-purple-900/40 pb-3">
                        <div>
                            ...
                            <p class="text-[11px] text-purple-400 font-mono">Showing {{ users_list|length }} of {{ users_total }} total registered accounts</p>
                        </div>
...
async def tenant_admin_panel(
    ...
    try:
        ...
        return HTMLResponse(f"""
            <div style="background:#070b14;color:#fff;font-family:sans-serif;padding:30px;">
                <h2 style="color:#f43f5e;">Template Render Exception</h2>
                <pre style="background:#111;padding:15px;border-radius:10px;color:#f87171;">{err}</pre>
            </div>
        """, status_code=500)
...
async def tenant_upload_demo_video(slug: str, request: Request, files: list[UploadFile] = File(...)):
    ...
    try:
        ...
        logging.error(f"[{slug}] Multiple upload error: {e}")
...
async def tenant_detect_upi_name(slug: str, upi_id: str, request: Request):
    ...
    return JSONResponse(content={"status": "failed", "message": "Live NPCI lookup blocked. Enter name manually."})
